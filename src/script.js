...
const gameOverEl = document.getElementById('gameOver'); // gets the id from html
...
const tick = () =>
{
  ...
  if (cube.position.x < -5) { // Decrease if you want the cubes to pass the donut
    const lastCube = obstacles.reduce((prev, current) =>
      prev.position.x > current.position.x ? prev : current
    )

    cube.position.x = lastCube.position.x + 3 // keep distance (3 = spacing)
  }
})

   // Game over function with smaller collider
const donutBox = new THREE.Box3().setFromCenterAndSize(
    mesh.position,                  // center at donut
    new THREE.Vector3(0.6, 0.6, 0.6) // size smaller than full torus
);
...
for (const cube of obstacles) {
    ...
    if (donutBox.intersectsBox(cubeBox)) {
        gameOverEl.style.display = 'block'; // show GAME OVER
